          ...
          <div className="flex-1 overflow-y-auto">
            <div className="p-4 sm:p-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                    {availableThemes.map((theme) => (
                      <motion.div
                        ...
                        className={`relative p-3 sm:p-4 border-2 rounded-2xl cursor-pointer transition-all hover:shadow-elegant ${currentTheme.id === theme.id
              ...
              {activeTab === 'fonts' && (
                <div>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-2">
                    <h3 className="text-base sm:text-lg font-semibold text-gray-900 font-display">{t('themeCustomizer.fontCollections')}</h3>
                    <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-600 font-sans">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                    {availableFonts.map((fontCollection) => (
                      <motion.div
                        ...
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        className={`relative p-4 sm:p-6 border-2 rounded-2xl cursor-pointer transition-all hover:shadow-elegant ${currentTheme.fonts.primary === fontCollection.fonts.primary
                          ? 'border-primary-500 bg-primary-50 shadow-glow'
                          : 'border-gray-200 hover:border-gray-300'
                          }`}
                      ...
                      >
                        {currentTheme.fonts.primary === fontCollection.fonts.primary && (
                          <motion.div
                            initial={{ scale: 0 }}
                            animate={{ scale: 1 }}
                            className="absolute top-3 right-3 w-5 h-5 sm:w-6 sm:h-6 bg-primary-600 rounded-full flex items-center justify-center shadow-glow"
                          >
                            <Check className="w-2 h-2 sm:w-3 sm:h-3 text-white" />
                          </motion.div>
                        )}
                        ...
                        <div className="mb-3 sm:mb-4">
                          <h4 className="font-semibold text-gray-900 text-base sm:text-lg mb-1 font-display">{fontCollection.name}</h4>
                          <p className="text-xs sm:text-sm text-gray-600 font-sans">{fontCollection.description}</p>
                        </div>
